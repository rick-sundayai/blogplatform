...
export default function AuthForm({ type }: AuthFormProps) {
  ...
  return (
    <div className="max-w-md w-full mx-auto">
      ...
      {/* Status message */}
      {message && (
        <div className={`p-4 mb-6 rounded-md ${message.type === 'success' ? 'bg-green-50 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-50 text-red-800 dark:bg-red-900 dark:text-red-200'}`}>
          {message.text}
        </div>
      )}
        ...
        {type !== 'resetPassword' && (
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className={`w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 ${errors.email ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}`}
      ...
      {config.footerText && (
        <div className="mt-6 text-center">
          <p className="text-gray-600 dark:text-gray-400">
            {config.footerText}{' '}
            <Link href={config.footerLink} className="text-indigo-600 dark:text-indigo-400 hover:underline">
